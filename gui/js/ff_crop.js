...
let OUTPUT_FILENAME = "ff_crop.mp4";
let LOGLEVEL = "error";
let WIDTH = "300";
let HEIGHT = "300";
let XPIXELS = "(iw-ow)/2";
...
function usage() {
    ...
    console.log("Summary:");
    console.log("Change the dimensions of the video.\n");
    ...
    console.log("\tWidth of the output video. Default: 300px.\n");

    console.log(" -h | --height <HEIGHT>");
    console.log("\tHeight of the output video. Default: 300px.\n");

    console.log(" -x | --xpixels <PIXELS>");
    console.log("\tWhere to position the video in the frame on X-Axis from left. Default center: (iw-ow)/2\n");

    console.log(" -y | --ypixels <PIXELS>");
    console.log("\tWhere to position the video in the frame on Y-Axis from top. Default center: (ih-oh)/2\n");
...
function arguments() {
    ...
    // Check if input file was provided (equivalent to JavaScript script's usage check)
...
function printFlags() {
    console.log(`📏 ${colors.TEXT_GREEN_400}%-10s :${colors.TEXT_RESET} %s`, "Width", WIDTH);
    console.log(`📐 ${colors.TEXT_GREEN_400}%-10s :${colors.TEXT_RESET} %s`, "Height", HEIGHT);
    console.log(`⏩ ${colors.TEXT_GREEN_400}%-10s :${colors.TEXT_RESET} %s`, "XPixels", XPIXELS);
    console.log(`⏫ ${colors.TEXT_GREEN_400}%-10s :${colors.TEXT_RESET} %s`, "YPixels", YPIXELS);
}
...
function main() {
    ...
    if (fs.statSync(INPUT_FILENAME).isFile()) {
        ...
        const ffmpegArgs = [
            '-loglevel', LOGLEVEL,
            '-i', INPUT_FILENAME,
            '-vf', `crop=w=${WIDTH}:h=${HEIGHT}:x=${XPIXELS}:y=${YPIXELS}`,
            OUTPUT_FILENAME
        ];

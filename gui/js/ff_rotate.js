...
let OUTPUT_FILENAME = "ff_rotate.mp4";
let LOGLEVEL = "error";
let ROTATION = "90";
let GREP = "";
...
// ╰──────────────────────────────────────────────────────────╯
function usage() {
    console.log(`ℹ️ Usage: node ff_rotate.js -i <INPUT_FILE> [-r <ROTATION>] [-o <OUTPUT_FILE>] [-l loglevel] [-g <GREP>] [-C <CONFIG_FILE>]\n`);
    
    console.log("Summary:");
    console.log("Rotate the video x amount of degrees.\n");
    ...
    console.log(" -r | --rotation <DEGREES>");
    console.log("\tDefault is 90\n");
...
function readConfig() {
    ...
    try {
        ...
        // Handle nested structure (e.g., {"ff_rotate": {...}})
        ...
        // Apply config values
        if (config.input) {
            // Use original config directory if available, otherwise fall back to temp file directory
            const configDir = process.env.SCRIPTFLOW_CONFIG_DIR || path.dirname(CONFIG_FILE);
            INPUT_FILENAME = path.resolve(configDir, config.input);
        }
        if (config.output) OUTPUT_FILENAME = config.output;
        if (config.rotation) ROTATION = config.rotation;
...
function preFlightChecks(inputFile) {
    ...
    if (isNaN(rotationNum)) {
        console.log(`\t❌ Invalid rotation value: '${ROTATION}'. Must be a number. Exiting.`);
        exitGracefully();
    }
...
function printFlags() {
    console.log(`📐 ${styles.green}%-10s :${styles.reset} %s`, "Rotation", ROTATION);
}
...
async function main() {
    ...
    if (stats.isFile()) {
        ...
        ffmpeg.on('close', (code) => {
            if (code === 0) {
                console.log(`✅ ${styles.purple}%-10s :${styles.reset} %s`, "Output", OUTPUT_FILENAME);
            } else {
    ...
    if (stats.isDirectory()) {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            ffmpeg.on('close', (code) => {
                if (code === 0) {
                    console.log(`✅ ${styles.purple}%-10s :${styles.reset} %s`, "Output", outputFile);
                } else {
                    console.error(`FFmpeg process exited with code ${code}`);
                }
            });

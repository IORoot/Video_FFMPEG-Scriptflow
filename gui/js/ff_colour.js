...
let OUTPUT_FILENAME = "ff_colour.mp4";
let BRIGHTNESS = "0";      // -1.0 to 1.0
let CONTRAST = "1";        // -1000.0 to 1000.0
let GAMMA = "1";           // 0.1 to 10.0
let SATURATION = "1";      // 0.0 to 3.0
let WEIGHT = "1";          // 0.0 to 1.0
let LOGLEVEL = "error";
let GREP = "";
...
function usage() {
    ...
    console.log("Summary:");
    console.log("Change the Brightness, Contrast, Gamma, Gamma-Weight and Saturation of a video.\n");
    ...
    console.log(" -b | --brightness <BRIGHTNESS>");
    console.log("\tChange the brightness value from -1.0 to 1.0.\n");

    console.log(" -c | --contrast <CONTRAST>");
    console.log("\tChange the contrast value from -1000.0 to 1000.0.\n");

    console.log(" -m | --gamma <GAMMA>");
    console.log("\tChange the gamma value from 0.1 to 10.0.\n");

    console.log(" -s | --saturation <SATURATION>");
    console.log("\tChange the saturation value from 0.0 to 3.0.\n");

    console.log(" -w | --weight <GAMMAWEIGHT>");
    console.log("\tChange the gamma weight value from 0.0 to 1.0.\n");
...
function readConfig(configFile) {
    try {
        ...
        if (config.brightness) BRIGHTNESS = config.brightness;
        if (config.contrast) CONTRAST = config.contrast;
        if (config.gamma) GAMMA = config.gamma;
        if (config.saturation) SATURATION = config.saturation;
        if (config.weight) WEIGHT = config.weight;
...
function printFlags() {
    console.log(`💡 ${colors.TEXT_GREEN_400}%-10s :${colors.TEXT_RESET} %s`, "Brightness", BRIGHTNESS);
    console.log(`🔲 ${colors.TEXT_GREEN_400}%-10s :${colors.TEXT_RESET} %s`, "Contrast", CONTRAST);
    console.log(`🎚️  ${colors.TEXT_GREEN_400}%-10s :${colors.TEXT_RESET} %s`, "Gamma", GAMMA);
    console.log(`🌈 ${colors.TEXT_GREEN_400}%-10s :${colors.TEXT_RESET} %s`, "Saturation", SATURATION);
    console.log(`🏋️  ${colors.TEXT_GREEN_400}%-10s :${colors.TEXT_RESET} %s`, "Weight", WEIGHT);
}
...
function main() {
    ...
    if (fs.statSync(INPUT_FILENAME).isFile()) {
        ...
        const ffmpegArgs = [
            '-v', LOGLEVEL,
            '-i', INPUT_FILENAME,
            '-vf', `eq=brightness=${BRIGHTNESS}:contrast=${CONTRAST}:gamma=${GAMMA}:saturation=${SATURATION}:gamma_weight=${WEIGHT}`,
            '-c:a', 'copy',
            OUTPUT_FILENAME
        ];

...
function keywordSubstitutions(scriptContents, configDirectory) {
    ...
    // VAR="ABC" node gui/js/scriptflow.js
...
function cleanup() {
    if (TIDY) {
        try {
            ...
            // Note: We no longer clean up video files automatically to preserve source inputs
            // Individual scripts should handle their own intermediate file cleanup
...
async function runFfScript(scriptName, scriptConfig, configDirectory) {
    ...
    // Run script - JavaScript version only
    ...
    if (configDirectory.includes('scriptflow_tests')) {
        ...
        jsScriptPath = path.join(configDirectory, '..', '..', '..', '..', 'gui', 'js', `${actualScriptName}.js`);
    } else if (configDirectory.includes('gui') || configDirectory.endsWith('Video_FFMPEG-Scriptflow')) {
        // Running from GUI or project root
        jsScriptPath = path.join(configDirectory, 'gui', 'js', `${actualScriptName}.js`);
    } else {
    ...
    if (fs.existsSync(jsScriptPath)) {
        ...
        throw new Error(`JavaScript script not found: ${actualScriptName} (checked ${jsScriptPath})`);
    }

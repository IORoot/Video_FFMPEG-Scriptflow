...
let OUTPUT_FILENAME = "ff_middle.mp4";
let TRIM = "1";
let LOGLEVEL = "error";
let GREP = "";
...
function usage() {
    ...
    console.log(" $0 -i <INPUT_FILE> [-t <TRIM>] [-o <OUTPUT_FILE>] [-l loglevel]\n");

    console.log("Summary:");
    console.log("Trim input video from start and end by a number of seconds.\n");
    ...
    console.log(" -t | --trim <TRIM>");
    console.log(`\tNumber of seconds to remove from the start and end of video. Default is ${TRIM} second.\n`);
...
function readConfig(configFile) {
    try {
        ...
        if (config.trim) TRIM = config.trim;
...
function printFlags() {
    console.log(`🔪 ${colors.TEXT_GREEN_400}%-10s :${colors.TEXT_RESET} %s`, "Trim", TRIM);
}
...
function main() {
    ...
    if (fs.existsSync(INPUT_FILENAME) && fs.statSync(INPUT_FILENAME).isFile()) {
        ...
        let fileDuration;
        try {
            fileDuration = parseFloat(execSync(`ffprobe -v ${LOGLEVEL} -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 "${INPUT_FILENAME}"`, { 
                encoding: 'utf8',
                stdio: 'pipe'
            }).trim());
        } catch (error) {
            console.error(`Error getting file duration: ${error.message}`);
            process.exit(1);
        }

        // Calculate trim times
        const halfTrimFromStart = parseFloat(TRIM) / 2;
        const start = formatTime(halfTrimFromStart);
        const halfTrimFromEnd = fileDuration - halfTrimFromStart;
        const end = formatTime(halfTrimFromEnd);
        ...
        ffmpeg.on('close', (code) => {
            if (code !== 0) {
                ...
                let newFileDuration;
                try {
                    newFileDuration = execSync(`ffprobe -v ${LOGLEVEL} -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 "${outputWithExt}"`, { 
                        encoding: 'utf8',
                        stdio: 'pipe'
                    }).trim();
                } catch (error) {
                    newFileDuration = "unknown";
                }
                
                console.log(`✅ ${colors.TEXT_PURPLE_500}%-10s :${colors.TEXT_RESET} %s`, "Output", outputWithExt);
                console.log(`✅ %-10s ( ⏲️  new duration: %s)`, outputWithExt, newFileDuration);
            }
        });
    ...
    if (fs.existsSync(INPUT_FILENAME) && fs.statSync(INPUT_FILENAME).isDirectory()) {
        ...
        files.forEach((inputFile, index) => {
            ...
            let fileDuration;
            try {
                fileDuration = parseFloat(execSync(`ffprobe -v ${LOGLEVEL} -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 "${inputFile}"`, { 
                    encoding: 'utf8',
                    stdio: 'pipe'
                }).trim());
            } catch (error) {

...
let OUTPUT_FILENAME = "ff_to_portrait.mp4";
let ROTATE = "1";
let LOGLEVEL = "error";
let GREP = "";
let CONFIG_FILE = "";
...
function usage() {
    if (process.argv.length < 3) {
        ...
        console.log("\t1 = 90Clockwise (default)");
        console.log("\t2 = 90CounterClockwise");
...
function readConfig() {
    ...
    try {
        ...
        // Extract ff_to_portrait config
        if (config.ff_to_portrait) {
            const portraitConfig = config.ff_to_portrait;
            
            if (portraitConfig.input) {
                if (path.isAbsolute(portraitConfig.input)) {
                    INPUT_FILENAME = portraitConfig.input;
                } else {
                    INPUT_FILENAME = path.resolve(path.dirname(CONFIG_FILE), portraitConfig.input);
                }
            }
            
            if (portraitConfig.output) OUTPUT_FILENAME = portraitConfig.output;
            if (portraitConfig.rotate) ROTATE = portraitConfig.rotate;
            if (portraitConfig.grep) GREP = portraitConfig.grep;
            if (portraitConfig.loglevel) LOGLEVEL = portraitConfig.loglevel;
        }
    } catch (error) {
...
async function main() {
    ...
    if (fs.existsSync(INPUT_FILENAME) && fs.statSync(INPUT_FILENAME).isFile()) {
        ...
        if (height > width) {
            console.log(`❌ ${INPUT_FILENAME} Already portrait (${width}x${height})`);
            exitGracefully();
        }
    ...
    if (fs.existsSync(INPUT_FILENAME) && fs.statSync(INPUT_FILENAME).isDirectory()) {
        ...
        for (const file of files) {
            ...
            if (height > width) {
                console.log(`❌ ${file} Already portrait (${width}x${height})`);
                continue;
            }

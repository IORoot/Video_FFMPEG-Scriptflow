...
let OUTPUT_FILENAME = "ff_convert.mp4";
let LOGLEVEL = "error";
let GREP = "";
let FORMAT = "mp4";
...
function usage() {
    ...
    console.log("Summary:");
    console.log("Convert an iPhone Quicktime MOV file to MP4.\n");
    ...
    console.log("\tOutput format. Default is 'mp4'.");
    console.log("\tOptions: mp4, mov, avi, webm, mkv\n");
...
function exitGracefully() {
    try {
        if (fs.existsSync(INPUT_FILENAME)) {
            const outputWithExt = `${OUTPUT_FILENAME.split('.')[0]}.${FORMAT}`;
            fs.copyFileSync(INPUT_FILENAME, outputWithExt);
        }
    } catch (error) {
...
function getCodecs() {
    switch (FORMAT) {
        ...
        case "mov":
            return ["-vcodec", "libx264", "-acodec", "aac"];
        case "avi":
            return ["-vcodec", "libx264", "-acodec", "mp3"];
        case "webm":
...
function printFlags() {
    ...
    console.log(`📤  ${colors.TEXT_GREEN_400}%-10s :${colors.TEXT_RESET} %s`, "Format", FORMAT);
}
...
function main() {
    ...
    if (fs.statSync(INPUT_FILENAME).isDirectory()) {
        ...
        files.forEach((file, index) => {
            ...
            const outputWithExt = `${index}_${OUTPUT_FILENAME.split('.')[0]}.${FORMAT}`;
